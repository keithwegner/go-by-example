...
// Timeouts are important for programs that connect to external resources or that otherwise need to bound
// execution time.
// Implementing timeouts in Go is easy and elegant thanks to channels and select.

func main() {

	// For our example, suppose we’re executing an external call that returns its result on a channel c1
	// after 2s. Note that the channel is buffered, so the send in the goroutine is nonblocking.
